...
static const char* SCATTERND_PLUGIN_NAME{"ScatterND_MoGo"};
...
__global__ void _ScatterNDKernel(const Dtype *updata_input, const int *indicesInputPtr , Dtype* output, int* flag,
    ...
    for(int idx=0; idx < channel_num; idx++){
        if (output[idx_output*channel_num+idx] < updata_input[idx_num*channel_num+idx]){
            // Dtype new_valu = updata_input[idx_num*channel_num+idx];
            // Dtype old_valu = 0;
            // do{
            //     old_valu = atomicExch(&output[idx_output*channel_num+idx], new_valu);
            //     new_valu = old_valu;
            // }while(output[idx_output*channel_num+idx] < new_valu);
...
int ScatterNDPlugin::enqueue(int batchSize, const void* const* inputs, void* const* outputs, void*, cudaStream_t stream)noexcept
{
    ...
    err = cudaMalloc((void **)&flag_cuda, sizeof(flag));
    if (err != cudaSuccess) {
        printf("cudaMalloc failed!");
        return 1;
    }
    cudaMemcpy(flag_cuda, flag, sizeof(flag), cudaMemcpyHostToDevice);
    ...
    case nvinfer1::DataType::kFLOAT:
        ...
        _ScatterNDKernel<<<gridsize, blockSize,0,stream>>>(static_cast<float const*> (inputs[2]), static_cast<int32_t const*> (inputs[1]), 
                                                    static_cast<float *> (outputs[0]), static_cast<int32_t *> (flag_cuda), channel_num, max_index_num);
        break;
    ...
    case nvinfer1::DataType::kHALF:
        ...
        _ScatterNDKernel<<<gridsize, blockSize,0,stream>>>(static_cast<int16_t const*> (inputs[2]), static_cast<int32_t const*> (inputs[1]), 
                                                    static_cast<int16_t *> (outputs[0]), static_cast<int32_t *> (flag_cuda), channel_num, max_index_num);
