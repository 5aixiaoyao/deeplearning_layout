...
void grid2points_gpu(at::Tensor points_tensor, at::Tensor indices_tensor, at::Tensor grid_tensor){
    ...
    int n_channels = points_tensor.size(1);
	grid2points_launcher(points, indices, grid, n, n_channels);
}

PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("forward", &grid2points_gpu, "grid to points (CUDA)");
}
